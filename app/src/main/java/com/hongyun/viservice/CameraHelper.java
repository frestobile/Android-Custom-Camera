...
public class CameraHelper {
    ...
    public static File getOutputMediaFile(String fileName, int type) {
        ...
        try {
            String recordedTime = new SimpleDateFormat("yyyyMMdd").format(new Date());
            Date currentdate = new SimpleDateFormat("yyyyMMdd").parse(recordedTime);

            File[] files = mediaStorageDir.listFiles();
            for (int i = 0; i < files.length; i++) {
                Log.e("name......",files[i].getName());
                Long lastmodified = files[i].lastModified();
                long purgeTime = System.currentTimeMillis() - (1 * 24 * 60 * 60 * 1000);

                if(lastmodified<purgeTime) {
                    files[i].delete();
                }
            }
