...
//            openCamera(width,height);
            opencameracameraforvideo(width, height);
...
//        }
     ...
     *
     * @param choices           The list of size1s that the camera supports for the intended output
     *                          class
...
//                        maxPreviewHeight, largest);
                ...
                Log.e("size array",map.getOutputSizes(SurfaceTexture.class).toString());
    ...
    private void opencameracameraforvideo(int width, int height){
        ...
        Log.e("width...................",""+mPreviewSize.getWidth()+"  "+mPreviewSize.getHeight());
        int orientation = getResources().getConfiguration().orientation;
        if (orientation == Configuration.ORIENTATION_LANDSCAPE) {
...
//        configureTransform(width, height);
    ...
    private void  createCameraPreviewSession() {
        if (mCameraDevice == null || mTextureView.getSurfaceTexture() == null
                || (mode == Values.MODE_IMAGE && mImageReader == null)) {
            return;
        }
        try {
...
//
                    ...
                    try {
                        ...
                        mPreviewSession.setRepeatingRequest(mPreviewRequest, imageCaptureCallback, mBackgroundHandler);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
    ...
    private void setOnFlash(CaptureRequest.Builder requestBuilder) {
        if (mFlashSupported) {
            ...
            try {
                CaptureRequest mPreviewRequest=requestBuilder.build();
                mPreviewSession.setRepeatingRequest(mPreviewRequest, imageCaptureCallback, mBackgroundHandler);
            } catch (Exception e) {
                e.printStackTrace();
            }
...
//        isSpeakButtonLongPressed = false;
    ...
    public void timerTick(long tickUpdateInMillis) {
        String formattedTime="";
            if(String.valueOf(TimeUnit.MILLISECONDS.toMinutes(tickUpdateInMillis)).length()==1) {
                 formattedTime = "0" + TimeUnit.MILLISECONDS.toMinutes(tickUpdateInMillis) + ":" +
                        TimeUnit.MILLISECONDS.toSeconds(tickUpdateInMillis) % 60;
            }
            if(String.valueOf(TimeUnit.MILLISECONDS.toSeconds(tickUpdateInMillis)% 60).length()==1)
            {
                formattedTime = "0" + TimeUnit.MILLISECONDS.toMinutes(tickUpdateInMillis) + ":0" +
                        TimeUnit.MILLISECONDS.toSeconds(tickUpdateInMillis) % 60;
            }
    ...
    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    private void startRecordingVideo(boolean init) {
        if (null == mCameraDevice || !mTextureView.isAvailable() || null == mPreviewSize) {
            Log.d("things called","return  called");
...
//            mPreviewBuilder.set(CaptureRequest.NOISE_REDUCTION_MODE, CaptureRequest.NOISE_REDUCTION_MODE_FAST);
    ...
    private void stopRecordingVideo() {
        ...
        createCameraPreviewSession();
            ...
            Intent i = new Intent(getContext(), PhotoVideoRedirectActivity.class);
